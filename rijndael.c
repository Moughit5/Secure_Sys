/*
 * Author: Abdelmoughit OUDINA
 * Student Number: D24130820
 *
 * Description:
 * This code implements the AES algorithm 
 * in C for 128-bit encryption and decryption. It includes key expansion, 
 * byte substitution, row shifting, column mixing, and round key addition.
 */

#include "rijndael.h"
#include <stdlib.h>
#include <string.h>
#include <stdio.h>

#define BLOCK_SIZE 16     // AES block size in bytes (128 bits)
#define NUM_ROUNDS 10     // Number of rounds for AES-128

/* 
 * Forward S-box for encryption
 * Used in SubBytes transformation
 */
static const unsigned char sbox[256] = {
    
};

/* 
 * Inverse S-box for decryption
 * Used in InvSubBytes transformation 
 */
static const unsigned char inv_sbox[256] = {
    
};

/*
 * Round constants for key expansion
 * Used in Rcon step of key schedule
 */
static const unsigned char Rcon[11] = {
  0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};


/**
 * Multiplies two numbers in GF(2^8) finite field
 * @param a First operand
 * @param b Second operand
 * @return Result of multiplication in GF(2^8)
 */
static unsigned char gf_mul(unsigned char a, unsigned char b) {
    unsigned char p = 0;
    unsigned char hi_bit_set;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        hi_bit_set = a & 0x80;
        a <<= 1;
        if (hi_bit_set) a ^= 0x1b;  // Réduction modulo x^8 + x^4 + x^3 + x + 1
        b >>= 1;
    }
    return p;
}

/**
 * Expands the 128-bit cipher key into 11 round keys (176 bytes)
 * @param cipher_key The original 128-bit cipher key
 * @return Pointer to expanded key schedule (176 bytes)
 */

unsigned char *expand_key(const unsigned char *cipher_key) {
  unsigned char *round_keys = malloc(176);
  if (!round_keys) return NULL;

  memcpy(round_keys, cipher_key, 16); // Copy original key

  for (int i = 4; i < 44; ++i) {
      unsigned char temp[4];
      memcpy(temp, round_keys + 4 * (i - 1), 4);

      if (i % 4 == 0) {
          // Rotate word
          unsigned char t = temp[0];
          temp[0] = temp[1];
          temp[1] = temp[2];
          temp[2] = temp[3];
          temp[3] = t;

          // SubWord
          for (int j = 0; j < 4; ++j) {
              temp[j] = sbox[temp[j]];
          }

          // Rcon
          temp[0] ^= Rcon[i / 4];
      }

      for (int j = 0; j < 4; ++j) {
          round_keys[4 * i + j] = round_keys[4 * (i - 4) + j] ^ temp[j];
      }
  }

  return round_keys;
}

/**
 * SubBytes transformation for encryption
 * @param block 16-byte block to transform
 */
void sub_bytes(unsigned char *block) {
    for (int i = 0; i < BLOCK_SIZE; i++) {
        block[i] = sbox[block[i]];
    }
}

// SubBytes inverse
void invert_sub_bytes(unsigned char *block) {
    for (int i = 0; i < BLOCK_SIZE; i++) {
        block[i] = inv_sbox[block[i]];
    }
}

/**
 * Inverse SubBytes transformation for decryption
 * @param block 16-byte block to transform
 */

void shift_rows(unsigned char *state) {
  unsigned char tmp[BLOCK_SIZE];

  tmp[0] = state[0];
  tmp[1] = state[5];
  tmp[2] = state[10];
  tmp[3] = state[15];

  tmp[4] = state[4];
  tmp[5] = state[9];
  tmp[6] = state[14];
  tmp[7] = state[3];

  tmp[8] = state[8];
  tmp[9] = state[13];
  tmp[10] = state[2];
  tmp[11] = state[7];

  tmp[12] = state[12];
  tmp[13] = state[1];
  tmp[14] = state[6];
  tmp[15] = state[11];

  memcpy(state, tmp, BLOCK_SIZE);
}


/**
 * Inverse ShiftRows transformation for decryption
 * @param state 16-byte state matrix to transform
 */

void invert_shift_rows(unsigned char *state) {
  unsigned char tmp[BLOCK_SIZE];

  tmp[0] = state[0];
  tmp[1] = state[13];
  tmp[2] = state[10];
  tmp[3] = state[7];

  tmp[4] = state[4];
  tmp[5] = state[1];
  tmp[6] = state[14];
  tmp[7] = state[11];

  tmp[8] = state[8];
  tmp[9] = state[5];
  tmp[10] = state[2];
  tmp[11] = state[15];

  tmp[12] = state[12];
  tmp[13] = state[9];
  tmp[14] = state[6];
  tmp[15] = state[3];

  memcpy(state, tmp, BLOCK_SIZE);
}


/**
 * MixColumns transformation for encryption
 * @param state 16-byte state matrix to transform
 */
void mix_columns(unsigned char *state) {
    unsigned char tmp[16];
    for (int i = 0; i < 4; i++) {
        tmp[4*i]   = gf_mul(0x02, state[4*i]) ^ gf_mul(0x03, state[4*i+1]) ^ state[4*i+2] ^ state[4*i+3];
        tmp[4*i+1] = state[4*i] ^ gf_mul(0x02, state[4*i+1]) ^ gf_mul(0x03, state[4*i+2]) ^ state[4*i+3];
        tmp[4*i+2] = state[4*i] ^ state[4*i+1] ^ gf_mul(0x02, state[4*i+2]) ^ gf_mul(0x03, state[4*i+3]);
        tmp[4*i+3] = gf_mul(0x03, state[4*i]) ^ state[4*i+1] ^ state[4*i+2] ^ gf_mul(0x02, state[4*i+3]);
    }
    memcpy(state, tmp, 16);
}

/**
 * Inverse MixColumns transformation for decryption
 * @param state 16-byte state matrix to transform
 */
void invert_mix_columns(unsigned char *state) {
    unsigned char tmp[16];
    for (int i = 0; i < 4; i++) {
        tmp[4*i]   = gf_mul(0x0e, state[4*i]) ^ gf_mul(0x0b, state[4*i+1]) ^ gf_mul(0x0d, state[4*i+2]) ^ gf_mul(0x09, state[4*i+3]);
        tmp[4*i+1] = gf_mul(0x09, state[4*i]) ^ gf_mul(0x0e, state[4*i+1]) ^ gf_mul(0x0b, state[4*i+2]) ^ gf_mul(0x0d, state[4*i+3]);
        tmp[4*i+2] = gf_mul(0x0d, state[4*i]) ^ gf_mul(0x09, state[4*i+1]) ^ gf_mul(0x0e, state[4*i+2]) ^ gf_mul(0x0b, state[4*i+3]);
        tmp[4*i+3] = gf_mul(0x0b, state[4*i]) ^ gf_mul(0x0d, state[4*i+1]) ^ gf_mul(0x09, state[4*i+2]) ^ gf_mul(0x0e, state[4*i+3]);
    }
    memcpy(state, tmp, 16);
}

/**
 * AddRoundKey transformation (shared between encryption and decryption)
 * @param block 16-byte block to transform
 * @param round_key 16-byte round key to add
 */
void add_round_key(unsigned char *block, const unsigned char *key) {
    for (int i = 0; i < BLOCK_SIZE; i++) {
        block[i] ^= key[i];
    }
}

/**
 * Encrypts a 128-bit block using AES-128
 * @param plaintext 16-byte plaintext block
 * @param key 16-byte encryption key
 * @return Pointer to 16-byte ciphertext block (allocated on heap)
 */
unsigned char *aes_encrypt_block(const unsigned char *plaintext, const unsigned char *key) {
    if (!plaintext || !key) return NULL;

    unsigned char *round_keys = expand_key(key);
    if (!round_keys) return NULL;

    unsigned char *state = malloc(BLOCK_SIZE);
    if (!state) {
        free(round_keys);
        return NULL;
    }

    // Copie du texte clair dans l'état initial
    memcpy(state, plaintext, BLOCK_SIZE);

    // Premier AddRoundKey
    add_round_key(state, round_keys);

    // 9 rounds complets
    for (int round = 1; round < NUM_ROUNDS; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, round_keys + round * BLOCK_SIZE);
    }

    // Dernier round (sans MixColumns)
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, round_keys + NUM_ROUNDS * BLOCK_SIZE);

    free(round_keys);
    return state;
}


/**
 * Decrypts a 128-bit block using AES-128
 * @param ciphertext 16-byte ciphertext block
 * @param key 16-byte decryption key
 * @return Pointer to 16-byte plaintext block (allocated on heap)
 */
unsigned char *aes_decrypt_block(const unsigned char *ciphertext, const unsigned char *key) {
    if (!ciphertext || !key) return NULL;

    unsigned char *round_keys = expand_key(key);
    if (!round_keys) return NULL;

    unsigned char *state = malloc(BLOCK_SIZE);
    if (!state) {
        free(round_keys);
        return NULL;
    }

    // Copie du texte chiffré dans l'état initial
    memcpy(state, ciphertext, BLOCK_SIZE);

    // Premier AddRoundKey avec la dernière sous-clé
    add_round_key(state, round_keys + NUM_ROUNDS * BLOCK_SIZE);

    // 9 rounds complets inverses
    for (int round = NUM_ROUNDS-1; round > 0; round--) {
        invert_shift_rows(state);
        invert_sub_bytes(state);
        add_round_key(state, round_keys + round * BLOCK_SIZE);
        invert_mix_columns(state);
    }

    // Dernier round inverse (sans InvMixColumns)
    invert_shift_rows(state);
    invert_sub_bytes(state);
    add_round_key(state, round_keys);

    free(round_keys);
    return state;
}